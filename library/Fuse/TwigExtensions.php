...
class Fuse_TwigExtensions extends Twig_Extension implements \Fuse\InjectionAwareInterface
{
    protected $di;
    ...
    {
        return array(
            'alink'     => new Twig_Filter_Function(array($this, 'twig_bf_admin_link_filter'), array('is_safe' => array('html'))),
            'link'      => new Twig_Filter_Function(array($this, 'twig_bf_client_link_filter'), array('is_safe' => array('html'))),
            ...
            'bfmd' => new Twig_Filter_Function('twig_bfmd_filter', array('needs_environment' => true, 'is_safe' => array('html'))),

            'bf_date' => new Twig_Filter_Function(array($this, 'twig_bf_date')),
            'bf_datetime' => new Twig_Filter_Function(array($this, 'twig_bf_datetime')),
    ...
    public function twig_bf_date($time, $format = null)
    {
    ...
    public function twig_bf_datetime($time, $format = null)
    {
        $locale_date_format = $this->di['config']['locale_date_format'];
    ...
    function twig_bf_client_link_filter($link, $params = null)
    {
        return $this->di['url']->link($link, $params);
    }

    function twig_bf_admin_link_filter($link, $params = null)
    {
...
{
    return BF_URL . 'modules/'.ucfirst($mod).'/assets/'.$asset;
}

function twig_asset_url(Twig_Environment $env, $asset)
{
    $globals = $env->getGlobals();
    return BF_URL . 'templates/'.$globals['current_theme'].'/assets/'.$asset;
}
...
{
    return sprintf('<script type="text/javascript" src="%s?%s"></script>', $path, Fuse_Version::VERSION);
}

function twig_stylesheet_tag($path, $media = 'screen')
{
    return sprintf('<link rel="stylesheet" type="text/css" href="%s?v=%s" media="%s" />', $path, Fuse_Version::VERSION, $media);
}

function twig_gravatar_filter($email, $size = null)
{
	$protocol = BF_SSL ? 'https' : 'http';
...
/**
 * BillingFuse markdown
 */
function twig_bfmd_filter(Twig_Environment $env, $value)
{
