...
class Fuse_Validate
{
    ...
    public function isEmailValid($email, $throw = true)
    {
        ...
        if(!$valid && $throw) {
            throw new \Fuse_Exception('Email is not valid');
        }
        return $valid;
    }
    ...
    {
        if( strlen($pwd) < 7 ) {
            throw new \Fuse_Exception("Password too short!");
        }

        if( !preg_match("#[0-9]+#", $pwd) ) {
            throw new \Fuse_Exception("Password must include at least one number!");
        }

        if( !preg_match("#[a-z]+#", $pwd) ) {
            throw new \Fuse_Exception("Password must include at least one letter!");
        }

        /*
        if( !preg_match("#[A-Z]+#", $pwd) ) {
            throw new \Fuse_Exception("Password must include at least one CAPS!");
        }

        if( !preg_match("#\W+#", $pwd) ) {
            throw new \Fuse_Exception("Password must include at least one symbol!");
        }
        */
        return true;
    }
    ...
    public function checkRequiredParamsForArray(array $required, array $data, array $variables = NULL, $code = 0)
    {
        foreach ($required as $key => $msg) {
            ...
            if(!isset($data[$key])){
                throw new \Fuse_Exception($msg, $variables, $code);
            }
    ...
    public function isBirthdayValid($birthday = '')
    {
        if (strlen(trim($birthday)) > 0 && strtotime($birthday) === false) {
            throw new \Fuse_Exception('Invalid birth date value');
        }
        return true;
    }
}
