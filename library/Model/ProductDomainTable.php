...
class Model_ProductDomainTable extends Model_ProductTable
{
    ...
    public function getProductPrice(Model_Product $product, array $config = array())
    {
        ...
        if(!isset($config['action'])) {
            throw new \Fuse__Exception('Could not determine domain price. Domain action is missing', null, 498);
        }
        ...
        if(!$tld instanceof Model_Tld) {
            throw new \Fuse__Exception('Unknown TLD. Could not determine registration price');
        }
    ...
    {
        throw new \Fuse__Exception('Domain product can not be removed.');
    }
}