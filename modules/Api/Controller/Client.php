...
namespace Fuse\Mod\Api\Controller;
...
class Client implements InjectionAwareInterface
{
    ...
    public function show_error(\Fuse_App $app, $page)
    {
        $exc = new \Fuse_Exception('Unknown API call :call', array(':call'=>$page), 879);
        return $this->renderJson(null, $exc);
    }
    
    public function get_method(\Fuse_App $app, $role, $class, $method)
    {
        $call = $class.'_'.$method;
        return $this->tryCall($role, $call, $_GET);
    }

    public function post_method(\Fuse_App $app, $role, $class, $method)
    {
        $p = $_POST;
    ...
    private function checkRateLimit()
    {
        ...
        if ($requests_left < 0) {
            throw new \Fuse_Exception('Request limit reached', null, 1003);
        }
        return true;
    }
    ...
    {
        // snake oil: check request is from the same domain as BillingFuse is installed if present
        ...
        if($check_referer_header) {
            $url = strtolower(BF_URL);
            $referer = isset($_SERVER['HTTP_REFERER']) ? strtolower($_SERVER['HTTP_REFERER']) : null ;
            if(!$referer || $url != substr($referer, 0, strlen($url))) {
                throw new \Fuse_Exception('Invalid request. Make sure request origin is :from', array(':from'=>BF_URL), 1004);
            }
        }
        return true;
    }
    ...
    private function checkAllowedIps()
    {
        ...
        if(!empty($ips) && !in_array($this->_getIp(), $ips)) {
            throw new \Fuse_Exception('Unauthorized IP', null, 1002);
        }
        return true;
    }
    ...
    private function getAuth()
    {
        ...
        if(!isset($_SERVER['PHP_AUTH_USER'])) {
            throw new \Fuse_Exception('Authentication Failed', null, 201);
        }

        if(!isset($_SERVER['PHP_AUTH_PW'])) {
            throw new \Fuse_Exception('Authentication Failed', null, 202);
        }

        if(empty($_SERVER['PHP_AUTH_PW'])) {
            throw new \Fuse_Exception('Authentication Failed', null, 206);
        }
        ...
        switch ($username) {
            case 'client':
                ...
                if(!$model instanceof \Model_Client) {
                    throw new \Fuse_Exception('Authentication Failed', null, 204);
                }
            ...
            case 'admin':
                ...
                if(!$model instanceof \Model_Admin) {
                    throw new \Fuse_Exception('Authentication Failed', null, 205);
                }
            ...
            default:
                throw new \Fuse_Exception('Authentication Failed', null, 203);
        }
    }
    ...
    private function isRoleAllowed($role)
    {
        ...
        if (!in_array($role, $allowed)){
            throw new \Fuse_Exception('Unknow API call', null, 701);
        }
        return true;
    }
    ...
    public function renderJson($data = NULL, \Exception $e = NULL)
    {
        ...
        header('X-BillingFuse-Version: '.\Fuse_Version::VERSION);
