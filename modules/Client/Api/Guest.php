...
class Guest extends \Api_Abstract
{
    ...
    public function create($data = array())
    {
        ...
        if(isset($config['allow_signup']) && !$config['allow_signup']) {
            throw new \Fuse_Exception('New registrations are temporary disabled');
        }
        ...
        if($service->clientAlreadyExists($email)) {
            throw new \Fuse_Exception('Email is already registered. You may want to login instead of registering.');
        }
        ...
        if (isset($config['require_email_confirmation']) && (int)$config['require_email_confirmation'] && !$client->email_approved) {
            throw new \Fuse_Exception('Account has been created. Please check your mailbox and confirm email address.', null,  7777);
        }
    ...
    public function login($data)
    {
        ...
        if(!$client instanceof \Model_Client ) {
            ...
            throw new \Fuse_Exception('Please check your login details', array(), 403);
        }
        
        if(isset($data['remember'])) {
            $email = $data['email'];
            $cookie_time = (3600 * 24 * 30); // 30 days
            $this->di['cookie']->set('FUSECLR', 'e='.base64_encode($email).'&p='.base64_encode($client->pass), time() + $cookie_time, '/');
        }
    ...
    public function reset_password($data)
    {
        ...
        if(!$c instanceof \Model_Client ) {
            throw new \Fuse_Exception('Email not found in our database');
        }
    ...
    public function confirm_reset($data)
    {
        ...
        if(!$reset instanceof \Model_ClientPasswordReset) {
            throw new \Fuse_Exception('The link have expired or you have already confirmed password reset.');
        }
