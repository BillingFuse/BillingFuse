...
class Service implements \Fuse\InjectionAwareInterface
{
    ...
    public function sendTemplate($data)
    {
        ...
        if (!isset($data['to']) && !isset($data['to_staff']) && !isset($data['to_client'])) {
            throw new \Fuse_Exception('Receiver is not defined. Define to or to_client or to_staff parameter');
        }
        $vars = $data;
        ...
        if (!$from){
            throw new \Fuse_Exception('From email address can not be empty');
        }
    ...
    private function _getDefaults($data)
    {
        ...
        $mod  = $matches[1];
        $path = BF_PATH_MODS . '/' . ucfirst($mod) . '/html_email/' . $code . '.phtml';
    ...
    private function _getVarsString()
    {
        ...
        $str .= 'This email template was automatically generated by BillingFuse extension.   ' . PHP_EOL;
        $str .= 'Template is ready to be modified.   ' . PHP_EOL;
        $str .= 'Email template is just like BillingFuse theme file.   ' . PHP_EOL;
    ...
    public function resend(\Model_ActivityClientEmail $email)
    {
        ...
        if (APPLICATION_ENV == 'testing') {
            if (BF_DEBUG) error_log('Skipping email sending in test environment');
            return true;
        }
    ...
    public function resetTemplateByCode($code)
    {
        ...
        if (!$t instanceof \Model_EmailTemplate) {
            throw new \Fuse_Exception('Email template :code was not found', array(':code' => $code));
        }
    ...
    {
        $pattern = BF_PATH_MODS . '/*/html_email/*.phtml';
        $list    = glob($pattern);
    ...
    private function _sendFromQueue(\Model_ModEmailQueue $queue)
    {
        ...
        if (!$this->di['license']->isPro()) {
            ...
            $queue->content .= 'Powered by BillingFuse. Client management, invoicing and support software. http://www.billingfuse.com/';
            $queue->content .= PHP_EOL;
        }
