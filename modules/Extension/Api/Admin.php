...
class Admin extends \Api_Abstract
{
    ...
    /**
     * Get list of extensions from extensions.billingfuse.com
     * which can be installed on current version of BillingFuse
     * 
    ...
    public function update_core($data)
    {
        ...
        if(!$updater->getCanUpdate()) {
            throw new \Fuse_Exception('You have latest version of BillingFuse. You do not need to update.', null, 930);
        }
        
        $new_version = $updater->getLatestVersion();
        $updater->performUpdate();
        
        $this->di['logger']->info('Updated BillingFuse from %s to %s', \Fuse_Version::VERSION, $new_version);
        return true;
    }
    ...
    /**
     * Completely remove extension from BillingFuse
     * 
     * @param string $type - extensions type: mod, theme, gateway ...
     * @param string $id - extension id
     * 
    ...
    /**
     * Universal method for BillingFuse extensions 
    ...
    private function _getExtension($data)
    {
        ...
        if(!$ext instanceof \Model_Extension) {
            throw new \Fuse_Exception('Extension not found');
        }
