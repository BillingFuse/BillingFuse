...
class Service implements InjectionAwareInterface
{
    ...
    public function update(\Model_Extension $model)
    {
        ...
        if($model->type == 'mod') {
            ...
            if(empty($latest)) {
                throw new \Fuse_Exception('Could not retrieve version information for extension :ext', array(':ext'=>$model->name), 745);
            }
            $haveUpdate = (version_compare($model->version, $latest) < 0);
            if(!$haveUpdate) {
                throw new \Fuse_Exception('Latest :mod version installed. No need to update', array(':mod'=>$model->name), 785);
            }

            $extension = $ext->getExtension($model->name);
            if(empty($extension)) {
                throw new \Fuse_Exception('Could not retrieve :ext information', array(':ext'=>$model->name), 744);
            }

            throw new \Fuse_Exception('Visit extension site for update information.', null, 252);
    ...
    {
        switch ($ext->type) {
            case \Fuse_Extension::TYPE_HOOK:
                $file = ucfirst($ext->name).'.php';
                $destination = BF_PATH_LIBRARY . '/Hook/'.$file;
            ...
            case \Fuse_Extension::TYPE_MOD:
                ...
                if($this->isCoreModule($mod)) {
                    throw new \Fuse_Exception("BillingFuse core modules can not be managed");
                }
    ...
    public function downloadAndExtract($type, $id)
    {
        ...
        $extracted = BF_PATH_CACHE.'/'.md5(uniqid());
        $zip = BF_PATH_CACHE.'/'.md5(uniqid()).'.zip';
        ...
        if ($res === TRUE) {
            ...
            throw new \Fuse_Exception('Could not extract extension zip file');
        }

        //install by type
        switch ($type) {
            case \Fuse_Extension::TYPE_MOD:
                $destination = BF_PATH_MODS . '/mod_'.$id;
                if($this->di['tools']->fileExists($destination)) {
                    throw new \Fuse_Exception('Module already installed.', null, 436);
                }
                if(!$this->di['tools']->rename($extracted, $destination)) {
                    throw new \Fuse_Exception('Extension can not be moved. Make sure your server write permissions to modules folder.', null, 437);
                }
                break;

            case \Fuse_Extension::TYPE_THEME:
                $destination = BF_PATH_THEMES . '/'.$id;
                if(!$this->di['tools']->fileExists($destination)) {
                    if(!$this->di['tools']->rename($extracted, $destination)) {
                        throw new \Fuse_Exception('Extension can not be moved. Make sure your server write permissions to templates folder.', null, 439);
                    }
                }
                break;

            case \Fuse_Extension::TYPE_TRANSLATION:
                $destination = BF_PATH_LANGS . '/'.$id.'/LC_MESSAGES';
                $this->di['tools']->emptyFolder($destination);
                if(!$this->di['tools']->fileExists($destination)) {
                    $this->di['tools']->mkdir($destination, 0777, true);
                }
                if(!$this->di['tools']->rename($extracted, $destination)) {
                    throw new \Fuse_Exception('Extension can not be moved. Make sure your server write permissions to locale folder.', null, 440);
                }
                break;

            default:
                throw new \Fuse_Exception('Extension does not support auto-install feature. Extension must be installed manually');
        }
    ...
    private function installModule(\Model_Extension $ext)
    {
        ...
        if($mod->isCore()) {
            throw new \Fuse_Exception("BillingFuse core modules can not be installed or removed");
        }

        $info = $mod->getManifest();
        if(isset($info['minimum_billingfuse_version']) && Fuse_Version::compareVersion($info['minimum_billingfuse_version']) > 0) {
            throw new \Fuse_Exception('Module can not be installed. It requires at least :min version of BillingFuse. You are using :v', array(':min'=>$info['minimum_billingfuse_version'], ':v'=>Fuse_Version::VERSION));
        }
