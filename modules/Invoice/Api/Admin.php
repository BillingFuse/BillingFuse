...
class Admin extends \Api_Abstract
{
    ...
    public function renewal_invoice($data)
    {
        ...
        if($model->price <= 0) {
            throw new \Fuse_Exception('Order :id is free. No need to generate invoice.', array(':id'=>$model->id));
        }
     ...
     * @optional int $gateway_id - Payment gateway ID on BillingFuse
     * @optional float $amount - Transaction amount
     * @optional string $currency - Currency code. Must be available on BillingFuse
     * @optional string $type - Currency code. Must be available on BillingFuse
     * @optional string $status - Transaction status on BillingFuse
     ...
     *
     * @param int $bf_invoice_id - BillingFuse invoice id
     * @param int $bf_gateway_id - BillingFuse gateway id
     * 
     ...
     * @optional string $txn_id - transaction id on payment gateway
     * @optional bool $skip_validation - makes params bf_invoice_id and bf_gateway_id optional
     * 
    ...
    public function subscription_create($data)
    {
        ...
        if($client->currency != $data['currency']) {
            throw new \Fuse_Exception('Client currency must match subscription currency. Check if clients currency is defined.');
        }
        $subscriptionService = $this->di['mod_service']('Invoice', 'Subscription');
    ...
    public function subscription_get($data)
    {
        ...
        if(!$model instanceof \Model_Subscription) {
            throw new \Fuse_Exception('Subscription not found');
        }
