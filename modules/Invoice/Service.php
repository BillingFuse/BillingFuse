...
class Service implements InjectionAwareInterface
{
    ...
    public static function onAfterAdminInvoicePaymentReceived(\Fuse_Event $event)
    {
    ...
    public static function onAfterAdminInvoiceApprove(\Fuse_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
        $service = $di['mod_service']('invoice');
    ...
    public static function onAfterAdminInvoiceReminderSent(\Fuse_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
        $service = $di['mod_service']('invoice');
    ...
    public function refundInvoice(\Model_Invoice $invoice, $note = null)
    {
        ...
        switch ($logic) {
            ...
            case 'negative_invoice':
                ...
                if($total <= 0) {
                    throw new \Fuse_Exception('Can not refund invoice with negative amount');
                }
    ...
    public function deleteInvoiceByClient(\Model_Invoice $model)
    {
        ...
        foreach($invoiceItem as $item) {
            if($item->type == \Model_InvoiceItem::TYPE_ORDER) {
                throw new \Fuse_Exception('Invoice is related to order #:id. Please cancel order first.', array(':id'=>$item->rel_id));
            }
        }
    ...
    public function generateForOrder(\Model_ClientOrder $order, $due_days = null)
    {
        ...
        if($order->price <= 0) {
            throw new \Fuse_Exception('Invoices are not generated for 0 amount orders');
        }
    ...
    {
        if(!$client->currency) {
            throw new \Fuse_Exception('You must have at least one active order before you can add funds so you cannot proceed at the current time!');
        }
        ...
        if($min_amount && $amount < $min_amount) {
            throw new \Fuse_Exception('Amount is not valid', null, 981);
        }

        if($max_amount && $amount > $max_amount) {
            throw new \Fuse_Exception('Amount is not valid', null, 982);
        }
    ...
    public function processInvoice(array $data)
    {
        ...
        if(!$invoice instanceof \Model_Invoice) {
            throw new \Fuse_Exception('Invoice not found', null, 812);
        }

        $gtw = $this->di['db']->load('PayGateway', $data['gateway_id']);
        if(!$gtw instanceof \Model_PayGateway) {
            throw new \Fuse_Exception('Payment method not found', null, 813);
        }

        if(!$gtw->enabled) {
            throw new \Fuse_Exception('Payment method not enabled', null, 814);
        }
    ...
    public function generatePDF($hash, $identity)
    {
        ...
        if (!$invoice instanceof \Model_Invoice) {
            throw new \Fuse_Exception('Invoice not found');
        }
        ...
        if (isset($company['logo_url']) && !empty($company['logo_url'])) {
            ...
            if (substr($url, -4) === '.png') {
                ...
                if ($img) {
                    ...
                    throw new \Fuse_Exception('Error converting image to .png');
                }
            }
        }
    ...
    {
        /**
         * @var \Fuse\Mod\System\Service $systemService
         */
        $systemService = $this->di['mod_service']('system');
