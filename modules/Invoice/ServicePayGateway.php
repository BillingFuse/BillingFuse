...
class ServicePayGateway implements InjectionAwareInterface
{
    ...
    public function getAvailable()
    {
        ...
        $pattern = BF_PATH_LIBRARY.'/Payment/Adapter/*.php';
        $adapters = array();
    ...
    public function install($code)
    {
        ...
        if(!in_array($code, $available)) {
            throw new \Fuse_Exception('Payment gateway is not available for installation.');
        }
    ...
    public function getPaymentAdapter(\Model_PayGateway $pg, \Model_Invoice $model = null, $optional = array())
    {
        ...
        if(!class_exists($class)) {
            throw new \Fuse_Exception("Payment gateway :adapter was not found", array(':adapter'=>$class));
        }
    ...
    {
        $class = $this->getAdapterClassName($pg);
        if(!file_exists(BF_PATH_LIBRARY.'/Payment/Adapter/'.$pg->gateway.'.php')) {
            throw new \Fuse_Exception("Payment gateway :adapter was not found", array(':adapter'=>$pg->gateway));
        }

        if(!class_exists($class)) {
            throw new \Fuse_Exception("Payment gateway class $class was not found");
        }
