...
class ServiceTransaction implements InjectionAwareInterface
{
    ...
    public function create(array $data)
    {
        ...
        if(!$skip_validation) {
            if(!isset($data['bf_invoice_id'])) {
                throw new \Fuse_Exception('Transaction invoice id is missing');
            }

            if(!isset($data['bf_gateway_id'])) {
                throw new \Fuse_Exception('Payment gateway id is missing');
            }
            $this->di['db']->getExistingModelById('Invoice', $data['bf_invoice_id'], 'Invoice was not found');
            $this->di['db']->getExistingModelById('PayGateway', $data['bf_gateway_id'], 'Gateway was not found');
        }
        ...
        $transaction->gateway_id    = $this->di['array_get']($data, 'bf_gateway_id', NULL);
        $transaction->invoice_id    = $this->di['array_get']($data, 'bf_invoice_id', NULL);
    ...
    public function processTransaction($id)
    {
        ...
        if(!$tx) {
            throw new \Fuse_Exception('Transaction :id not found.', array('id'=>$id), 404);
        }

        if(empty($tx->gateway_id)) {
            throw new \Fuse_Exception('Could not determine transaction origin. Transaction payment gateway is unknown.', null, 701);
        }

        $gtw = $this->di['db']->load('PayGateway', $tx->gateway_id);
        if(!$gtw instanceof \Model_PayGateway) {
            throw new \Fuse_Exception('Can not handle transaction received from unknown payment gateway: :id', array(':id'=>$tx->gateway_id), 704);
        }

        $payGatewayService = $this->di['mod_service']('Invoice', 'PayGateway');
        $adapter = $payGatewayService->getPaymentAdapter($gtw);
        if(!method_exists($adapter, 'processTransaction')) {
            throw new \Fuse_Exception('Payment adapter :adapter does not support action :action', array(':adapter'=>$gtw->name, ':action'=>'processTransaction'), 705);
        }
    ...
    public function process($tx)
    {
        ...
        try {
            ...
            switch ($transaction->type) {
                ...
                default:
                    throw new \Fuse_Exception('Unknown transaction #:id type: :type', array(':id'=>$transaction->id, ':type'=>$transaction->type), 632);
            }
        } catch(\Exception $e) {
            ...
            if(BF_DEBUG) error_log($e->getMessage());
    ...
    private function _parseIpnAndApprove(\Model_Transaction &$tx)
    {
        ...
        if(!$tx->invoice_id) {
            throw new \Fuse_Exception('Transaction :id is not associated with an invoice.', array(':id'=>$tx->id), 702);
        }

        $invoice = $this->di['db']->load('Invoice', $tx->invoice_id);
        if(!$invoice instanceof \Model_Invoice) {
            throw new \Fuse_Exception('Invoice #:id not found', array(':id'=>$tx->invoice_id), 703);
        }
        ...
        if(APPLICATION_ENV != 'testing' && $tx->validate_ipn) {
            if(!$adapter->isIpnValid($ipn, $mpi)) {
                ...
                throw new \Fuse_Exception('Instant payment notification (IPN) did not pass gateway :id validation', array(':id'=>$gtw->gateway), 706);
            }
            $tx->output        = $adapter->getOutput();
        }

        if(!method_exists($adapter, 'getTransaction')) {
            throw new \Fuse_Exception('Payment adapter :adapter does not support action :action', array(':adapter'=>$gtw->name, ':action'=>'getTransaction'), 705);
        }

        $response = $adapter->getTransaction($ipn, $mpi);
        if(!$response instanceof \Payment_Transaction) {
            throw new \Fuse_Exception('Payment gateway :id method getTransaction should return Payment_Transaction object', array(':id'=>$gtw->gateway), 705);
        }
    ...
    private function _subscribe(\Model_Transaction $tx)
    {
        ...
        if(empty($tx->s_id)) {
            throw new \Fuse_Exception('Can not create subscription. Subscription id from payment gateway was not received');
        }
    ...
    private function _unsubscribe(\Model_Transaction $tx)
    {
        ...
        if(!$model instanceof \Model_Subscription) {
            throw new \Fuse_Exception('Subscription #:id was not found. Could not unsubscribe', array(':id'=>$tx->s_id));
        }
    ...
    {
        if($tx->status != \Model_Transaction::STATUS_APPROVED) {
            throw new \Fuse_Exception('Only approved transaction can be processed');
        }

        if(empty($tx->invoice_id)) {
            throw new \Fuse_Exception('Transaction :id is not associated with an invoice.', array(':id'=>$tx->id), 7022);
        }
        ...
        if($invoice->currency != $tx->currency) {
            throw new \Fuse_Exception('Transaction currency :code do not match required currency :required', array(':code'=>$tx->currency, ':required'=>$invoice->currency), 709);
        }

        // check that payment status is completed if
        if($tx->txn_status == \Payment_Transaction::STATUS_PENDING) {
            throw new \Fuse_Exception('Transaction status on payment gateway is Pending. Only Complete or Unknown transactions can be processed.', null, 712);
        }
    }
    ...
    public function debitTransaction(\Model_Transaction $tx)
    {
        ...
        if($client->currency != $proforma->currency) {
            throw new \Fuse_Exception('Client currency do not match invoice currency');
        }

        // do not debit negative or zero amount
        if($tx->amount < 0) {
            throw new \Fuse_Exception('Can not add negative amount to client balance for debit transaction');
        }
