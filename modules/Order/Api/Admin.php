...
class Admin extends \Api_Abstract
{
    ...
    public function unsuspend($data)
    {
        ...
        if ($order->status != \Model_ClientOrder::STATUS_SUSPENDED) {
            throw new \Fuse_Exception('Only suspended orders can be unsuspended');
        }
    ...
    public function uncancel($data)
    {
        ...
        if ($order->status != \Model_ClientOrder::STATUS_CANCELED) {
            throw new \Fuse_Exception('Only canceled orders can be uncanceled');
        }
    ...
    public function update_config($data)
    {
        ...
        if (!isset($data['config']) || !is_array($data['config'])) {
            throw new \Fuse_Exception('Order config not passed');
        }
