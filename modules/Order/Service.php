...
class Service implements InjectionAwareInterface
{
    ...
    public static function onAfterAdminOrderActivate(\Fuse_Event $event)
    {
    ...
    public static function onAfterAdminOrderRenew(\Fuse_Event $event)
    {
        $params   = $event->getParameters();
        $order_id = $params['id'];
        $di = $event->getDi();
    ...
    public static function onAfterAdminOrderSuspend(\Fuse_Event $event)
    {
        $params   = $event->getParameters();
        $order_id = $params['id'];
        $di = $event->getDi();
    ...
    public static function onAfterAdminOrderUnsuspend(\Fuse_Event $event)
    {
        $params   = $event->getParameters();
        $order_id = $params['id'];
        $di = $event->getDi();
    ...
    public static function onAfterAdminOrderCancel(\Fuse_Event $event)
    {
        $params   = $event->getParameters();
        $order_id = $params['id'];
        $di = $event->getDi();
    ...
    {
        $log = $this->di['logger'];
        $log->addWriter(new \Fuse_LogDb('Model_ClientOrderStatus'));
    ...
    public function createOrder(\Model_Client $client, \Model_Product $product, array $data)
    {
        if (!$this->di['license']->isPro()) {
            throw new \Fuse_Exception('This feature is available in BillingFuse PRO version.', null, 876);
        }
        ...
        if (!$currency instanceof \Model_Currency) {
            throw new \Fuse_Exception('Currency could not be determined for order');
        }
        ...
        if (!$cartService->isStockAvailable($product, $qty)) {
            throw new \Fuse_Exception("Product :id is out of stock.", array(':id' => $product->id), 831);
        }

        // Addons must have defined master order
        $parent_order = FALSE;
        if ($product->is_addon && empty($group_id)) {
            throw new \Fuse_Exception('Group ID parameter is missing for addon product order', null, 832);
        }

        if (!empty($group_id)) {
            $parent_order = $this->getMasterOrderForClient($client, $group_id);
            if (!$parent_order instanceof \Model_ClientOrder) {
                throw new \Fuse_Exception('Parent order :group_id was not found', array(':group_id' => $group_id));
            }
        }
    ...
    public function activateOrder(\Model_ClientOrder $order, $data = array())
    {
        ...
        if (!in_array($order->status, $statues)){
            if (!isset($data['force']) || !$data['force']) {
                throw new \Fuse_Exception('Only pending setup or failed orders can be activated');
            }
        }
    ...
    public function createFromOrder(\Model_ClientOrder $order)
    {
        ...
        if (!is_object($service)) {
            ...
            if (method_exists($s, 'create') || method_exists($s, 'action_create')) {
                ...
                if (!is_object($service)) {
                    throw new \Fuse_Exception('Error creating ' . $order->service_type . ' service for order ' . $order->id);
                }
    ...
    protected function _callOnService(\Model_ClientOrder $order, $action)
    {
        ...
        if (in_array($order->service_type, $core_services)) {
            ...
            if (!method_exists($repo, $m) || !is_callable(array($repo, $m))) {
                throw new \Fuse_Exception('Service ' . $order->service_type . ' do not support ' . $m);
            }
    ...
    public function suspendFromOrder(\Model_ClientOrder $order, $reason = null, $skipEvent = false)
    {
        ...
        if ($order->status != \Model_ClientOrder::STATUS_ACTIVE) {
            throw new \Fuse_Exception('Only active orders can be suspended');
        }
    ...
    public function cancelFromOrder(\Model_ClientOrder $order, $reason = null, $skipEvent = false)
    {
        ...
        if (in_array($order->status, array(\Model_ClientOrder::STATUS_CANCELED, \Model_ClientOrder::STATUS_PENDING_SETUP, \Model_ClientOrder::STATUS_FAILED_SETUP))) {
            throw new \Fuse_Exception('Can not cancel ' . $order->status . ' order');
        }
