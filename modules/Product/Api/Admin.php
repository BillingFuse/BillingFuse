...
namespace Fuse\Mod\Product\Api;
...
class Admin extends \Api_Abstract
{
    ...
    public function prepare($data)
    {
        ...
        if($data['type'] == 'domain') {
            ...
            if($model instanceof \Model_Product) {
                throw new \Fuse_Exception('You have already created domain product.', null, 413);
            }
        }

        $types = $service->getTypes();
        if(!array_key_exists($data['type'], $types)) {
            throw new \Fuse_Exception('Product type :type is not registered', array(':type'=>$data['type']), 413);
        }
    ...
    public function update_priority($data)
    {
        if(!isset($data['priority']) || !is_array($data['priority'])) {
            throw new \Fuse_Exception('priority params is missing');
        }
    ...
    public function addon_get($data)
    {
        ...
        if(!$model instanceof \Model_Product || !$model->is_addon) {
            throw new \Fuse_Exception('Addon not found');
        }
        $service = $this->getService();
