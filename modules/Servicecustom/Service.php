...
namespace Fuse\Mod\Servicecustom;
...
class Service implements \Fuse\InjectionAwareInterface
{
            ...
            foreach ($form['fields'] as $field) {
                if ($field['required'] == 1) {
                    ...
                    if ((!isset($data[$field_name]) || empty($data[$field_name]))) {
                        throw new \Fuse_Exception("You must fill in all required fields. " . $field['label'] . " is missing", null, 9684);
                    }
                }
                ...
                if ($field['readonly'] == 1) {
                    ...
                    if ($data[$field_name] != $field['default_value']) {
                        throw new \Fuse_Exception("Field " . $field['label'] . " is read only. You can not change its value", null, 5468);
                    }
                }
            }
        }
    ...
    public function customCall(\Model_ServiceCustom $model, $method, $params = array())
    {
        ...
        if (in_array($method, $forbidden_methods)) {
            throw new \Fuse_Exception('Custom plugin method :method is forbidden', array(':method' => $method), 403);
        }
    ...
    {
        if (!is_array($config)){
            throw new \Fuse_Exception('Config must be an array');
        }
