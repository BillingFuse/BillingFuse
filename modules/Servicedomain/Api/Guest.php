...
namespace Fuse\Mod\Servicedomain\Api;
...
class Guest extends \Api_Abstract
{
    ...
    public function check($data)
    {
        ...
        if (!$validator->isSldValid($sld)) {
            throw new \Fuse_Exception('Domain :domain is not valid', array(':domain' => $sld));
        }

        $tld = $this->getService()->tldFindOneByTld($data['tld']);
        if (!$tld instanceof \Model_Tld) {
            throw new \Fuse_Exception('Domain availability could not be determined. TLD is not active.');
        }

        if (!$this->getService()->isDomainAvailable($tld, $sld)) {
            throw new \Fuse_Exception('Domain is not available.');
        }
    ...
    public function can_be_transferred($data)
    {
        ...
        $tld = $this->getService()->tldFindOneByTld($data['tld']);
        if (!$tld instanceof \Model_Tld) {
            throw new \Fuse_Exception('TLD is not active.');
        }
        if (!$this->getService()->canBeTransfered($tld, $data['sld'])) {
            throw new \Fuse_Exception('Domain can not be transferred.');
        }
