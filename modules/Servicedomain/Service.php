...
namespace Fuse\Mod\Servicedomain;
...
class Service implements \Fuse\InjectionAwareInterface
{
    ...
    public function validateOrderData(&$data)
    {
        ...
        if (!in_array($action, array('register', 'transfer', 'owndomain'))) {
            throw new \Fuse_Exception('Invalid domain action.');
        }

        if ($action == 'owndomain') {
            if (!isset($data['owndomain_sld'])) {
                throw new \Fuse_Exception('Order data must contain :field configuration field', array(':field' => 'owndomain_sld'));
            }

            if (!$validator->isSldValid($data['owndomain_sld'])) {
                throw new \Fuse_Exception('Domain name :domain is not valid', array(':domain' => $data['owndomain_sld']));
            }
        ...
        if ($action == 'transfer') {
            if (!isset($data['transfer_sld'])) {
                throw new \Fuse_Exception('Order data must contain :field configuration field', array(':field' => 'transfer_sld'));
            }

            if (!$validator->isSldValid($data['transfer_sld'])) {
                throw new \Fuse_Exception('Domain name :domain is not valid', array(':domain' => $data['transfer_sld']));
            }
            ...
            if (!$tld instanceof \Model_Tld) {
                throw new \Fuse_Exception('TLD not found');
            }

            $domain = $data['transfer_sld'] . $tld->tld;
            if (!$this->canBeTransfered($tld, $data['transfer_sld'])) {
                throw new \Fuse_Exception(':domain can not be transferred!', array(':domain' => $domain));
            }
        ...
        if ($action == 'register') {
            if (!isset($data['register_sld'])) {
                throw new \Fuse_Exception('Order data must contain :field configuration field', array(':field' => 'register_sld'));
            }

            if (!$validator->isSldValid($data['register_sld'])) {
                throw new \Fuse_Exception('Domain name :domain is not valid', array(':domain' => $data['register_sld']));
            }
            ...
            if ($years < $tld->min_years) {
                throw new \Fuse_Exception(':tld can be registered for at least :years years', array(':tld' => $tld->tld, ':years' => $tld->min_years));
            }

            $domain = $data['register_sld'] . $tld->tld;
            if (!$this->isDomainAvailable($tld, $data['register_sld'])) {
                throw new \Fuse_Exception(':domain is already registered!', array(':domain' => $domain));
            }
    ...
    public function action_create(\Model_ClientOrder $order)
    {
        ...
        if (empty($ns)) {
            throw new \Fuse_Exception('Default domain nameservers are not configured');
        }
    ...
    public function action_activate(\Model_ClientOrder $order)
    {
        ...
        if (!$model instanceof \Model_ServiceDomain) {
            throw new \Fuse_Exception('Could not activate order. Service was not created');
        }
    ...
    public function action_renew(\Model_ClientOrder $order)
    {
        ...
        if (!$model instanceof \RedBeanPHP\SimpleModel) {
            throw new \Fuse_Exception('Order :id has no active service', array(':id' => $order->id));
        }
        //@adapterAction
        list($domain, $adapter) = $this->_getD($model);
    ...
    {
        if (!isset($data['ns1'])) {
            throw new \Fuse_Exception('Nameserver 1 is required');
        }
        if (!isset($data['ns2'])) {
            throw new \Fuse_Exception('Nameserver 2 is required');
        }
    ...
    {
        if (empty($sld)) {
            throw new \Fuse_Exception('Domain name is not valid');
        }

        if (!$model->allow_transfer) {
            throw new \Fuse_Exception('Domain can not be transferred', null, 403);
        }
    ...
    public function isDomainAvailable(\Model_Tld $model, $sld)
    {
        ...
        if (!$validator->isSldValid($sld)) {
            throw new \Fuse_Exception('Domain name :domain is not valid', array(':domain' => $sld));
        }

        if (!$model->allow_register) {
            throw new \Fuse_Exception('Domain can not be registered', null, 403);
        }
    ...
    public function registrarGetAvailable()
    {
        ...
        $pattern  = BF_PATH_LIBRARY . '/Registrar/Adapter/*.php';
        $adapters = array();
    ...
    {
        if (!file_exists(BF_PATH_LIBRARY . '/Registrar/Adapter/' . $model->registrar . '.php')) {
            throw new \Fuse_Exception("Domain registrar :adapter was not found", array(':adapter' => $model->registrar));
        }

        $class = sprintf('Registrar_Adapter_%s', $model->registrar);
        if (!class_exists($class)) {
            throw new \Fuse_Exception("Registrar :adapter was not found", array(':adapter' => $class));
        }
    ...
    public function registrarGetRegistrarAdapter(\Model_TldRegistrar $r)
    {
        ...
        if (!$registrar instanceof \Registrar_AdapterAbstract) {
            throw new \Fuse_Exception('Registrar adapter :adapter should extend Registrar_AdapterAbstract', array(':adapter' => $class));
        }
    ...
    public function registrarRm(\Model_TldRegistrar $model)
    {
        ...
        if (count($domains) > 0) {
            throw new \Fuse_Exception('Can not remove registrar which has domains');
        }
