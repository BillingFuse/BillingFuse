...
namespace Fuse\Mod\Servicedownloadable;
...
class Service implements InjectionAwareInterface
{
    ...
    public function action_create(\Model_ClientOrder $order)
    {
        ...
        if (!is_array($c)){
            throw new \Fuse_Exception(sprintf('Order #%s config is missing', $order->id));
        }
    ...
    public function uploadProductFile(\Model_Product $productModel)
    {
        ...
        if ($request->hasFiles() == 0) {
            throw new \Fuse_Exception('Error uploading file');
        }
    ...
    public function sendFile(\Model_ServiceDownloadable $serviceDownloadable)
    {
        ...
        if(!$this->di['tools']->fileExists($path)) {
            throw new \Fuse_Exception('File can not be downloaded at the moment. Please contact support', null, 404);
        }
