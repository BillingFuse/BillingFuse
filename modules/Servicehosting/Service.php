...
namespace Fuse\Mod\Servicehosting;
...
class Service implements InjectionAwareInterface
{
    ...
    public function validateOrderData(array &$data)
    {
        if(!isset($data['server_id'])) {
            throw new \Fuse_Exception('Hosting product is not configured completely. Configure server for hosting product.', null, 701);
        }
        if(!isset($data['hosting_plan_id'])) {
            throw new \Fuse_Exception('Hosting product is not configured completely. Configure hosting plan for hosting product.', null, 702);
        }
        if(!isset($data['sld']) || empty($data['sld'])) {
            throw new \Fuse_Exception('Domain name is not valid.', null, 703);
        }
        if(!isset($data['tld']) || empty($data['tld'])) {
            throw new \Fuse_Exception('Domain extension is not valid.', null, 704);
        }
    }
    ...
    public function changeAccountUsername(\Model_ClientOrder $order, \Model_ServiceHosting $model, $data)
    {
        if(!isset($data['username']) || empty($data['username'])) {
            throw new \Fuse_Exception('Account password is missing or is not valid');
        }
    ...
    public function changeAccountIp(\Model_ClientOrder $order, \Model_ServiceHosting $model, $data)
    {
        if(!isset($data['ip']) || empty($data['ip'])) {
            throw new \Fuse_Exception('Account ip is missing or is not valid');
        }
    ...
    public function changeAccountDomain(\Model_ClientOrder $order, \Model_ServiceHosting $model, $data)
    {
        if(!isset($data['tld']) || empty($data['tld']) ||
           !isset($data['sld']) || empty($data['sld'])) {
            throw new \Fuse_Exception('Domain sld or tld is missing');
        }
    ...
    {
        $dir = BF_PATH_LIBRARY . '/Server/Manager';
        $files = array();
    ...
    {
        $filename = BF_PATH_LIBRARY . '/Server/Manager/'.$manager.'.php';
    ...
    public function getServerManager(\Model_ServiceHostingServer $model)
    {
        if(empty($model->manager)) {
            throw new \Fuse_Exception('Invalid server manager. Server was not configured properly.', null, 654);
        }
        ...
        if(!$manager instanceof \Server_Manager) {
            throw new \Fuse_Exception('Server manager :adapter is not valid', array(':adapter'=>$model->manager));
        }
    ...
    public function deleteHp(\Model_ServiceHostingHp $model)
    {
        ...
        if($serviceHosting) {
            throw new \Fuse_Exception('Can not remove hosting plan which has active accounts');
        }
        $this->di['db']->trash($model);
    ...
    public function getDomainProductFromConfig(\Model_Product $product, array &$data)
    {
        ...
        if(!$d instanceof \Model_Product) {
            throw new \Fuse_Exception('Could not find main domain product');
        }
