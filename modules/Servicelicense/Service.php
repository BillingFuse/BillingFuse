...
namespace Fuse\Mod\Servicelicense;
...
class Service implements InjectionAwareInterface
{
    ...
    public function action_activate(\Model_ClientOrder $order)
    {
        ...
        if(!is_object($plugin)) {
            throw new \Fuse_Exception('License plugin :plugin was not found', array(':plugin'=>$model->plugin));
        }

        if(!method_exists($plugin, 'generate')) {
            throw new \Fuse_Exception('License plugin do not have generate method');
        }
        ...
        do {
            ...
            if ($i++ >= $iterations) {
                throw new \Fuse_Exception('Maximum number of iterations reached while generating license key');
            }
    ...
    private function _getPlugin(\Model_ServiceLicense $model)
    {
        ...
        foreach($plugins as $plugin) {
            if($model->plugin == $plugin['filename']) {
                ...
                $class_name = 'Fuse\\Mod\\Servicelicense\\Plugin\\'.$model->plugin;
    ...
    {
        $log = $this->di['logger'];
        $log->addWriter(new \Fuse_LogStream(BF_PATH_LOG.'/license.log'));
        if($this->di['config']['debug']) $log ->debug(print_r($data, 1));

        /**
         * Return error code in result field if related to license error
         * If error comes from BillingFuse core use $result['error'] field
         * @since v2.7.1
         */
