...
class Admin extends \Api_Abstract
{
    ...
    /**
     * Import selected servers to BillingFuse
    ...
    public function import_servers($data)
    {
        ...
        if(empty($nodeid)) {
            throw new \Fuse_Exception('Node is not selected for import.', null, 235);
        }
        
        if(empty($selected)) {
            throw new \Fuse_Exception('No servers selected for import.', null, 234);
        }

        if(empty($product_id)) {
            throw new \Fuse_Exception('Select product for orders.');
        }
        ...
        $this->di['logger']->info('Imported VPS from SolusVM to BillingFuse');
        return $log;
    }
    
    /**
     * Import selected clients to BillingFuse
     * @return string - log information regarding import process
     */
    public function import_clients($data)
    {
        ...
        if(empty($selected)) {
            throw new \Fuse_Exception('No clients selected for import.', null, 233);
        }
        
        $clients = $this->client_list($data);
        if(empty($clients)) {
            throw new \Fuse_Exception('No clients found on SolusVM server.');
        }
        ...
        $this->di['logger']->info('Imported clients from SolusVM to BillingFuse');
        return $log;
    }
