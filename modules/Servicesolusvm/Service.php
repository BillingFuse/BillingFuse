...
namespace Fuse\Mod\Servicesolusvm;
...
class Service implements InjectionAwareInterface
{
    ...
    public function validateOrderData(array $data)
    {
        if (!isset($data['hostname']) || empty($data['hostname'])) {
            throw new \Fuse_Exception('Please enter VPS hostname.', null, 7101);
        }

        $ValidHostnameRegex = "/^(([a-zA-Z]|[a-zA-Z][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z]|[A-Za-z][A-Za-z0-9\-]*[A-Za-z0-9])$/";
        if (!preg_match($ValidHostnameRegex, $data['hostname'])) {
            throw new \Fuse_Exception('Hostname :hostname is not valid.', array(':hostname' => $data['hostname']), 7102);
        }

        if (!isset($data['template']) || empty($data['template'])) {
            throw new \Fuse_Exception('Please select VPS template.', null, 7103);
        }
    }
    ...
    {
        if (!isset($data['id'])) {
            throw new \Fuse_Exception('SolusVM API ID is missing.', null, 7201);
        }

        if (!isset($data['key'])) {
            throw new \Fuse_Exception('SolusVM API KEY is missing.', null, 7202);
        }

        if (!isset($data['ipaddress'])) {
            throw new \Fuse_Exception('SolusVM API ipaddress is missing.', null, 7203);
        }

        if (!isset($data['secure'])) {
            throw new \Fuse_Exception('SolusVM API secure is missing.', null, 7204);
        }

        if (!isset($data['port'])) {
            throw new \Fuse_Exception('SolusVM API port is missing.', null, 7204);
        }
    ...
    public function activate($order, $model)
    {
        ...
        if (!$product) {
            throw new \Fuse_Exception('Could not activate order because ordered product does not exists', null, 7457);
        }
        ...
        if (!isset($pconfig['node'])) {
            throw new \Fuse_Exception('Could not activate order. Product :id configuration is missing node parameter', array(':id' => $product->id), 7458);
        }

        if (!isset($pconfig['plan'])) {
            throw new \Fuse_Exception('Could not activate order. Product :id configuration is missing plan parameter', array(':id' => $product->id), 7459);
        }

        if (!isset($pconfig['vtype'])) {
            throw new \Fuse_Exception('Could not activate order. Product :id configuration is missing virtualization type parameter', array(':id' => $product->id), 7460);
        }

        if (!isset($pconfig['ips'])) {
            throw new \Fuse_Exception('Could not activate order. Product :id configuration is missing ips amount parameter', array(':id' => $product->id), 7461);
        }
        ...
        $model->cluster_id      = 1; //for future if ever BillingFuse supports multiple master servers
    ...
    public function set_root_password($order, $model, $params = array())
    {
        if (!isset($params['password']) || strlen($params['password']) < 4) {
            throw new \Fuse_Exception('Root password must be longer than 4 symbols', null, 8789);
        }
    ...
    public function set_plan($order, $model, $params = array())
    {
        if (!isset($params['plan']) || empty($params['plan'])) {
            throw new \Fuse_Exception('Plan name must not be empty', null, 8790);
        }
    ...
    public function set_hostname($order, $model, $params = array())
    {
        if (!isset($params['hostname']) || empty($params['hostname'])) {
            throw new \Fuse_Exception('Hostname must not be empty', null, 8791);
        }
    ...
    public function rebuild($order, $model, $params = array())
    {
        if (!isset($params['template']) || empty($params['template'])) {
            throw new \Fuse_Exception('Template must not be empty', null, 8792);
        }
    ...
    {
        if (!isset($params['password']) || strlen($params['password']) < 4) {
            throw new \Fuse_Exception('Password must be longer than 4 symbols', null, 8790);
        }
