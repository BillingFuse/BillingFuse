...
namespace Fuse\Mod\Serviceyouhosting;
class Service implements \Fuse\InjectionAwareInterface
{
    ...
    {
        $this->mod = new \Fuse_Mod('serviceyouhosting');
    }
    ...
    public function getCartProductTitle($product, array $data)
    {
        if ($data['domain_type'] == 'subdomain') {
            if (!isset($data['subdomain']) || empty($data['subdomain'])) {
                throw new \Fuse_Exception('Please enter subdomain', null, 7102);
            }

            if (!isset($data['master_domain']) || empty($data['master_domain'])) {
                throw new \Fuse_Exception('Please select master domain', null, 7103);
            }
        ...
        if ($data['domain_type'] == 'domain') {
            if (!isset($data['domain'])) {
                throw new \Fuse_Exception('Please enter domain value', null, 7104);
            }
    ...
    public function validateOrderData(array &$data)
    {
        if (!isset($data['domain_type']) || empty($data['domain_type'])) {
            throw new \Fuse_Exception('Unknown domain type.', null, 7101);
        }
    ...
    public function isYHClientBFClient($yh_client_id)
    {
        $id = $this->di['db']->getCell("
        SELECT client_id
        FROM extension_meta
    ...
    {
        if ($this->isYHClientBFClient($yh_client_data['id'])) {
        ...
        if ($clientService->emailAreadyRegistered($yh_client_data['email'])) {
            throw new \Fuse_Exception('Email is already registered.');
        }
    ...
    public function importYouhostingAccount($account)
    {
        ...
        $client_id = $this->isYHClientBFClient($account['client_id']);
        if (!$client_id) {
    ...
    public function getApi()
    {
        if (!$this->di['license']->isPro()) {
            throw new \Exception('YouHosting module can only be used by PRO license owners. Get PRO license key at http://www.billingfuse.com/order', 509);
        }
        ...
        $a = array(
            ...
            'billingfuse' => array('license' => $this->di['config']['license']),
        );
