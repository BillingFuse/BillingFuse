...
namespace Fuse\Mod\Spamchecker;
...
class Service implements InjectionAwareInterface
{
    ...
    public function forumSpamChecker(\Fuse_Event $event)
    {
        $di = $event->getDi();
    ...
    public static function onBeforeClientCreateForumTopic(\Fuse_Event $event)
    {
        $di = $event->getDi();
        $spamCheckerService = $di['mod_service']('Spamchecker');
    ...
    public static function onBeforeClientRepliedInForum(\Fuse_Event $event)
    {
        $di = $event->getDi();
        $spamCheckerService = $di['mod_service']('Spamchecker');
        $spamCheckerService->forumSpamChecker($event);
    ...
    public static function onBeforeClientSignUp(\Fuse_Event $event)
    {
        $di = $event->getDi();
        $spamCheckerService = $di['mod_service']('Spamchecker');
        $spamCheckerService->isBlockedIp($event);
    ...
    public function isBlockedIp($event)
    {
        ...
        if(isset($config['block_ips']) && $config['block_ips'] && isset($config['blocked_ips'])) {
            ...
            if(in_array($di['tools']->getIpv4(), $blocked_ips)) {
                throw new \Fuse_Exception('IP :ip is blocked', array(':ip'=>$di['tools']->getIpv4()), 403);
            }
        }
    }
    ...
    public function isCommentSpam($event, $comment)
    {
        ...
        require_once BF_PATH_MODS . '/Spamchecker/akismet.curl.class.php';
        ...
        if($akismet->is_spam($comment)) {
            throw new \Fuse_Exception('Akismet detected this message is spam');
        }
    }
    ...
    public function isInStopForumSpamDatabase(array $data)
    {
        ...
        if(isset($json->username->appears) && $json->username->appears) {
            throw new \Fuse_Exception('Your Username is blacklisted in global database');
        }
        if(isset($json->email->appears) && $json->email->appears) {
            throw new \Fuse_Exception('Your Email is blacklisted in global database');
        }
        if(isset($json->ip->appears) && $json->ip->appears) {
            throw new \Fuse_Exception('Your IP is blacklisted in global database');
        }
