...
class Service implements InjectionAwareInterface
{
    ...
    public function login($email, $password, $ip)
    {
        ...
        if(!$model instanceof \Model_Admin ) {
            ...
            throw new \Fuse_Exception('Check your login details', null, 403);
        }
    ...
    public static function onAfterClientReplyTicket(\Fuse_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
    ...
    public static function onAfterClientCloseTicket(\Fuse_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
    ...
    public static function onAfterGuestPublicTicketReply(\Fuse_Event $event)
    {
        $params = $event->getParameters();
        $di = $event->getDi();
    ...
    {
        if($model->protected) {
            throw new \Fuse_Exception('This administrator account is protected and can not be removed');
        }
    ...
    public function create(array $data)
    {
        ...
        try {
            ...
            throw new \Fuse_Exception('Staff member with email :email is already registered', array(':email'=>$data['email']), 788954);
        }
    ...
    public function deleteGroup(\Model_AdminGroup $model)
    {
        $id = $model->id;
        if($model->id == 1) {
            throw new \Fuse_Exception('Administrators group can not be removed');
        }
        ...
        if($staffMembersInGroup > 0) {
            throw new \Fuse_Exception('Can not remove group which has staff members');
        }
