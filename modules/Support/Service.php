...
class Service implements \Fuse\InjectionAwareInterface
{
    ...
    public static function onAfterAdminOpenTicket(\Fuse_Event $event)
    {
        $di = $event->getDi();
        $supportService = $di['mod_service']('support');
        $emailService = $di['mod_service']('email');
    ...
    public static function onAfterAdminCloseTicket(\Fuse_Event $event)
    {
        $di = $event->getDi();
        $supportService = $di['mod_service']('support');
        $emailService = $di['mod_service']('email');
    ...
    public static function onAfterAdminReplyTicket(\Fuse_Event $event)
    {
        $di = $event->getDi();
        $supportService = $di['mod_service']('support');
        $emailService = $di['mod_service']('email');
    ...
    public static function onAfterGuestPublicTicketOpen(\Fuse_Event $event)
    {
        $di = $event->getDi();
        $supportService = $di['mod_service']('support');
        $emailService = $di['mod_service']('email');
    ...
    public static function onAfterAdminPublicTicketOpen(\Fuse_Event $event)
    {
        $di = $event->getDi();
        $supportService = $di['mod_service']('support');
        $emailService = $di['mod_service']('email');
    ...
    public static function onAfterAdminPublicTicketReply(\Fuse_Event $event)
    {
        $di = $event->getDi();
        $supportService = $di['mod_service']('support');
        $emailService = $di['mod_service']('email');
    ...
    public function findOneByClient(\Model_Client $c, $id)
    {
        ...
        if (!$ticket instanceof \Model_SupportTicket) {
            throw new \Fuse_Exception('Ticket not found');
        }
    ...
    public function helpdeskRm(\Model_SupportHelpdesk $model)
    {
        ...
        if (count($tickets) > 0) {
            throw new \Fuse_Exception('Can not remove helpdesk which has tickets');
        }
        $this->di['db']->trash($model);
    ...
    public function canClientSubmitNewTicket(\Model_Client $client, array $config)
    {
        ...
        if ($timeSinceLast < $hours) {
            throw new \Fuse_Exception(sprintf('You can submit one ticket per %s hours. %s hours left', $hours, $hours - $timeSinceLast));
        }
    ...
    public function ticketCreateForClient(\Model_Client $client, \Model_SupportHelpdesk $helpdesk, array $data)
    {
        ...
        if (!is_null($rel_id) && $rel_type == \Model_SupportTicket::REL_TYPE_ORDER) {
            ...
            if (!$o instanceof \Model_ClientOrder) {
               throw new \Fuse_Exception('Order ID does not exist');
            }
        }
        ...
        if ($rel_id && $rel_type && $rel_task && $this->checkIfTaskAlreadyExists($client, $rel_id, $rel_type, $rel_task)) {
            throw new \Fuse_Exception('We have already received this request.');
        }
    ...
    public function messageCreateForTicket(\Model_SupportTicket $ticket, $identity, $content)
    {
        ...
        if ($identity instanceof \Model_Admin) {
            ...
            throw new \Fuse_Exception('Identity is not valid');
        }
        $msg->content    = $content;
        $msg->ip         = $this->di['request']->getClientAddress();
        $msg->created_at = date('Y-m-d H:i:s');
    ...
    public function publicFindOneByHash($hash)
    {
        ...
        if (!$publicTicket instanceof \Model_SupportPTicket) {
            throw new \Fuse_Exception('Public ticket not found');
        }
