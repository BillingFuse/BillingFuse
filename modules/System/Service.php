...
class Service
{
    ...
    {
        if(empty($param)) {
            throw new \Fuse_Exception('Parameter key is missing');
        }
    ...
    public function getMessages($type)
    {
        ...
        try {
            ...
            if($updater->getCanUpdate()) {
                ...
                $msgs['info'][] = sprintf('BillingFuse %s is available for download.', $version);
            }
        } catch(\Exception $e) {
        ...
        $install = BF_PATH_ROOT.'/install';
        if($this->di['tools']->fileExists(BF_PATH_ROOT.'/install')) {
            $msgs['info'][] = sprintf('Install module "%s" still exists. Please remove it for security reasons.', $install);
        }

        if(!extension_loaded('mcrypt')) {
            $msgs['info'][] = sprintf('BillingFuse requires %s extension to be enabled on this server for security reasons.', 'php mcrypt');
        }
    ...
    {
        $p = \Fuse_Period::getPredefined();
        if(isset($p[$code])) {
            return $p[$code];
        }

        $p = new \Fuse_Period($code);
        return $p->getTitle();
    }
    ...
    public function getPublicParamValue($param)
    {
        ...
        $results = $stmt->fetchColumn();
        if($results === false) {
            throw new \Fuse_Exception('Parameter :param does not exist', array(':param'=>$param));
        }
        return $results;
    }
    ...
    public function getPhoneCodes($data)
    {
        ...
        if(isset($data['country'])) {
            if(array_key_exists($data['country'], $codes)) {
                ...
                throw new \Fuse_Exception('Country :code phone code is not registered', array(':code'=>$data['country']));
            }
        }
    ...
    public function checkLimits($model, $limit = 2)
    {
        if (!$this->di['license']->isPro()) {
            ...
            if ($count >= $limit) {
                throw new \Fuse_Exception('You have reached free version limit. Upgrade to PRO version of BillingFuse if you want this limit removed.', null, 875);
            }
        }
    }
